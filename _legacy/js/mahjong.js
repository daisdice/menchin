...
const Mahjong = {
    ...
    // options: { minWaits: number, maxWaits: number, exactWaits: number }
    generateChinitsuHand: function (options = {}) {
        let hand;
        let waits;
        let attempts = 0;
        ...
        do {
            ...
            if (isValid) {
                if (options.exactWaits && waits.length !== options.exactWaits) isValid = false;
                if (options.minWaits && waits.length < options.minWaits) isValid = false;
                if (options.maxWaits && waits.length > options.maxWaits) isValid = false;
            }
        ...
        return this.generateChinitsuHand(); // Recursive call without options as fallback
    },
